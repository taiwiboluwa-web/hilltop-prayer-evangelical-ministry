...
if(!s.includes("GalleryMomentsManager")) s=s.replace("import { supabase } from '../supabase';", "import { supabase } from '../supabase';\nimport { GalleryMomentsManager } from '../components/GalleryMomentsManager';")
...
const modernRender="  const render=()=>tab==='overview'?renderOverview():tab==='ministers'?<AdminMinisterManager/>:tab==='gallery'?<GalleryMomentsManager/>:tab==='videos'?<AdminContentManager kind=\"videos\"/>:tab==='live'?<AdminContentManager kind=\"live\"/>:tab==='audio'?<AdminContentManager kind=\"audio\"/>:<AdminEvents onClose={()=>setTab('overview')}/>;"
s=s.replace(renderLine,modernRender)

// If a previous build already replaced the render line, make sure Gallery is
// never redirected to AdminContentManager. The grouped uploader is canonical.
s=s.replace("tab==='gallery'?<AdminContentManager kind=\"gallery\"/>", "tab==='gallery'?<GalleryMomentsManager/>")
...
console.log('Admin workspace upgraded: events, full CRUD, multi-photo minister editor, live image previews, and grouped Gallery Moments')
