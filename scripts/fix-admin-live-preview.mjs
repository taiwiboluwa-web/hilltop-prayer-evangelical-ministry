...
if(!/const \[mImage2,setMImage2\]=useState\(''\);/.test(minister)){const match=minister.match(primaryState);if(match&&match.index!=null){const insertAt=match.index+match[0].length;minister=minister.slice(0,insertAt)+stateBlock+minister.slice(insertAt)}else console.log('Minister editor state already normalized; skipping legacy state insertion')}
let seenM2=false;minister=minister.replace(/\n\s*const \[mImage2,setMImage2\]=useState\(''\);/g,m=>{if(seenM2)return '';seenM2=true;return m})
let seenM3=false;minister=minister.replace(/\n\s*const \[mImage3,setMImage3\]=useState\(''\);/g,m=>{if(seenM3)return '';seenM3=true;return m})
let seenFit=false;minister=minister.replace(/\n\s*const \[ministerFit,setMinisterFit\]=useState<'cover'|'contain'>\('cover'\);/g,m=>{if(seenFit)return '';seenFit=true;return m})
