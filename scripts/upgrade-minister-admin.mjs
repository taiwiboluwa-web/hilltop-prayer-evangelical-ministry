...
// JSX uses ${...} extensively. The temporary __D__ token prevents Node from
// evaluating those expressions while this script constructs the source.
const ministerBlock = [
  ...
  `      <div className="flex flex-wrap gap-3 mt-3"><button className="flex-1 min-w-[180px] h-11 rounded-full bg-[#d5aa49] text-[#09090b] text-xs font-bold tracking-[.12em] uppercase">__D__{ministers.some(m=>(m.display_order??m.id)===selectedMinister)?'Save Changes':'Add Minister'}</button><button type="button" onClick={()=>removeMinisterSlot(selectedMinister)} className=__D__{`px-5 h-11 rounded-full text-xs __D__{danger}`}>Remove Slot</button></div>`,
  `    </form>`,
  ...
  `      <div className="space-y-2 max-h-[620px] overflow-y-auto pr-1">__D__{ministerSlots.map(n=>{const m=ministers.find(x=>(x.display_order??x.id)===n);return <div key={n} className=__D__{`flex items-center gap-3 p-3 rounded-xl border __D__{selectedMinister===n?'border-[#c9a84c]/35 bg-[#c9a84c]/5':'border-white/10 bg-white/[.02]'}`}>`,
