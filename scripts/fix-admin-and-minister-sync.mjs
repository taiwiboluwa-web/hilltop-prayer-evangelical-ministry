...
if (!css.includes('Sidebar width compatibility: support both')) {
  fs.writeFileSync(adminCss, css + compatibility)
}
...
// Real database rows are still authoritative wherever the public page already loads them.
...
console.log('Admin sidebar compatibility and three-slot public minister fallback applied')
