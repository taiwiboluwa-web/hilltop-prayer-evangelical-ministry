...
import siteConfiguration from './bluehavens/make/site.json'
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  function sanitizeHtmlValue(value: string | undefined): string { return value?.replace(/[^a-zA-Z0-9_-]/g, '') || '' }
  function escapeHtmlText(value: string): string { return value.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;') }
  function replaceHtmlCommentSlot(html: string, slotName: string, content: string): string { return html.replace(`<!-- ${slotName} -->`, content) }
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        res.setHeader('Content-Type', 'text/plain; charset=utf-8'); res.end(robotsTxt)
      })
    },
    generateBundle() {
    ...
    transformIndexHtml: {
      ...
      handler(html) {
        ...
        if (socialImage) tags.push(
          { tag: 'meta', attrs: { property: 'og:image', content: socialImage }, injectTo: 'head' },
          { tag: 'meta', attrs: { name: 'twitter:card', content: 'summary_large_image' }, injectTo: 'head' },
          { tag: 'meta', attrs: { name: 'twitter:image', content: socialImage }, injectTo: 'head' },
        )
        if (googleAnalyticsId) tags.push(
        ...
        if (config.accessibility?.addBypassLinks) tags.push(
...
function figmaErrorOverlayReplay(): Plugin {
  return {
    name: 'figma-error-overlay-replay', apply: 'serve',
    configureServer(server) {
...
function figmaReactRefreshBoundaryFallback(): Plugin {
  ...
  return {
    name: 'figma-react-refresh-boundary-fallback', apply: 'serve', enforce: 'post',
...
function figmaMakeKitPlugin(options: { storiesGlob: string | string[] }): Plugin {
  ...
  const ROUTE = '/bluehavens/make/kit.html'
  ...
  return {
    name: 'figma-make-kit', apply: 'serve',
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        try { res.setHeader('Content-Type', 'text/html'); res.end(await server.transformIndexHtml(req.url || ROUTE, HTML_BOOTSTRAP)) } catch (err) { next(err as Error) }
      })
    },
  }
}
