...
function routeToPage(pathname: string) { const normalized = pathname.replace(/\/$/, '') || '/'; return PUBLIC_ROUTES[normalized] || PUBLIC_ROUTES[decodeURI(normalized)] || 'Home' }
function Site() {
  const [locationHost, setLocationHost] = useState<HTMLElement | null>(null); const [path, setPath] = useState(() => window.location.pathname)
  const normalizedPath = path.replace(/\/+$/, '') || '/'; const isAdminRoute = /^(?:\/admin(?:\/.*)?|\/adminaccess|\/adminpanel|\/adminportal|\/admindashboard)$/i.test(normalizedPath); const publicPage = routeToPage(path)
  ...
  return <><Analytics /><AdminSecurity /><SeasonalTheme admin={isAdminRoute} /><ChristmasStoryLayer admin={isAdminRoute} /><SiteCountdowns admin={isAdminRoute} />{!isAdminRoute && <EmojiIconReplacer />}{isAdminRoute ? <><AdminPortal onBack={goHome} /><GalleryMomentsEnhancer /></> : <><App />{locationHost && createPortal(<LocationDirections />, locationHost)}{publicPage === 'Events' && <GalleryMomentsEnhancer publicEvents />}</>}</>
}
