  ...
  if (!isAdmin) {
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4 font-sans">
        <div className="w-full max-w-[380px] bg-[#121212] border border-[#262626] rounded-xl p-6 shadow-2xl">
          <div className="flex justify-between items-center mb-5">
            <h2 className="text-white text-center text-base font-bold m-0 tracking-wide">Admin Portal Access</h2>
            <button onClick={onBack} className="bg-neutral-900 border border-neutral-800 text-neutral-300 px-2.5 py-1 rounded-md cursor-pointer text-xs font-medium hover:bg-neutral-800 transition-all">
              ← Back
            </button>
          </div>
          
          {authError && (
            <div className="bg-red-500/10 border border-red-500/30 text-red-400 p-2.5 rounded-lg text-xs mb-4 leading-relaxed">
              {authError}
            </div>
          )}
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-3">
            <div>
              <label className="block text-[#f59e0b] text-[10px] font-bold mb-1.5 tracking-[0.1em] uppercase">Email Address</label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full px-3 py-2.5 bg-neutral-900 border border-neutral-800 rounded-lg text-white box-border outline-none text-xs focus:border-[#f59e0b] transition-all" placeholder="admin@ministry.com" />
            </div>
            <div>
              <label className="block text-[#f59e0b] text-[10px] font-bold mb-1.5 tracking-[0.1em] uppercase">Password</label>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full px-3 py-2.5 bg-neutral-900 border border-neutral-800 rounded-lg text-white box-border outline-none text-xs focus:border-[#f59e0b] transition-all" placeholder="••••••••" />
            </div>
            <button type="submit" className="py-2.5 bg-[#f59e0b] text-[#0a0a0a] border-none rounded-lg font-bold cursor-pointer mt-1 text-xs tracking-wider shadow-md shadow-amber-500/10 hover:bg-amber-400 transition-all">
      ...
      <aside className={`bg-[#121212] border-r border-[#262626] flex flex-col justify-between fixed top-0 bottom-0 left-0 z-20 transition-all duration-300 ease-in-out ${isSidebarCollapsed ? 'w-16' : 'w-48'}`}>
        <div>
          ...
          <div className="p-2 space-y-1 overflow-y-auto">
            {!isSidebarCollapsed && (
              <span className="px-2.5 text-[9px] font-semibold uppercase tracking-wider text-neutral-500 mb-1.5 block">Views</span>
            )}
            {[
          ...
          <div className={`bg-neutral-900/60 border border-neutral-800/80 rounded-lg p-2 flex items-center justify-between ${isSidebarCollapsed ? 'justify-center p-1.5' : ''}`}>
            {!isSidebarCollapsed && (
              <div className="truncate mr-1.5">
                <span className="text-[10px] text-neutral-400 block truncate">{email || 'taiwiboluwa@gmail.com'}</span>
              </div>
            )}
            <button
      ...
      <main className={`flex-1 flex flex-col h-screen overflow-hidden bg-[#0a0a0a] transition-all duration-300 ${isSidebarCollapsed ? 'ml-16' : 'ml-48'}`}>
        ...
        <header className="h-14 border-b border-[#262626] bg-[#121212] px-5 flex items-center justify-between shrink-0">
          <div className="flex items-center space-x-3">
            <button
              onClick={onBack}
              className="text-[11px] font-semibold text-neutral-400 hover:text-white transition flex items-center space-x-1.5 cursor-pointer bg-neutral-900 border border-neutral-800 px-2.5 py-1 rounded-md"
            >
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
              <span>Back to Home</span>
            </button>
            <span className="text-neutral-700">/</span>
            <h1 className="text-[11px] font-bold tracking-wider text-white uppercase">
          ...
          <div className="flex items-center gap-2.5">
            <button
              ...
              className={`px-3 py-1 rounded-md font-bold cursor-pointer flex items-center gap-1.5 text-[11px] transition-all border ${
        ...
        <div className="p-4 overflow-y-auto flex-1">
          ...
          {uploadProgress && (
            <div className="bg-[#f59e0b]/10 border border-[#f59e0b]/30 text-[#f59e0b] p-2.5 rounded-lg text-xs mb-4 text-center font-medium shadow-md">
              {uploadProgress}
            </div>
          )}
              ...
              <div className="md:col-span-4 lg:col-span-4 bg-[#121212] border border-[#262626] p-4 rounded-xl shadow-xl flex flex-col">
                ...
                <div className="space-y-2.5 overflow-y-auto max-h-[420px] pr-1">
                  {galleryItems.length === 0 ? (
                    <p className="text-neutral-500 text-xs py-6 text-center">No gallery photos added yet.</p>
                  ) : (
                    galleryItems.map((item, index) => (
                      <div key={item.id} className="bg-neutral-900/60 border border-neutral-800/80 p-2.5 rounded-lg flex items-center justify-between gap-2.5">
                        <div className="flex items-center space-x-2.5 min-w-0">
                          <img src={item.image_url} alt="" className="w-10 h-8 rounded-md object-cover border border-neutral-800 shrink-0" />
                          <div className="truncate">
                            <span className="text-[9px] text-neutral-500 block">#{index + 1}</span>
                        ...
                        <button
                          ...
                          className="bg-red-500/10 border border-red-500/30 text-red-400 px-2.5 py-1 rounded-md text-[9px] font-bold uppercase hover:bg-red-500/20 transition cursor-pointer shrink-0"
                        >
                          Remove
                        </button>
                      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
              <form onSubmit={handleAddSermon} className="md:col-span-8 lg:col-span-8 bg-[#121212] border border-[#262626] p-4 rounded-xl shadow-xl flex flex-col justify-between">
                <div>
                  ...
                  <label className="flex items-center space-x-2 text-neutral-300 text-[11px] cursor-pointer mt-1">
                    <input type="checkbox" checked={vIsFeatured} onChange={(e) => setVIsFeatured(e.target.checked)} className="w-3.5 h-3.5 accent-[#f59e0b] rounded cursor-pointer" />
                    <span>Mark as Main Featured Sermon</span>
                  </label>
                </div>

                <div className="pt-4 mt-4 border-t border-[#262626]">
                  <button type="submit" className="w-full bg-[#f59e0b] text-[#0a0a0a] font-bold text-[11px] uppercase tracking-wider py-2.5 rounded-lg hover:bg-amber-400 transition shadow-md shadow-amber-500/10 cursor-pointer">
                    Publish Sermon
                  </button>
                </div>
              </form>
              ...
              <div className="md:col-span-4 lg:col-span-4 bg-[#121212] border border-[#262626] p-4 rounded-xl shadow-xl flex flex-col">
                ...
                <div className="space-y-2.5 overflow-y-auto max-h-[420px] pr-1">
                  {sermons.length === 0 ? (
                    <p className="text-neutral-500 text-xs py-6 text-center">No video sermons uploaded yet.</p>
                  ) : (
                    sermons.map((s) => (
                      <div key={s.id} className="bg-neutral-900/60 border border-neutral-800/80 p-3 rounded-lg flex justify-between items-center gap-2.5">
                        <div className="min-w-0 truncate">
                          <p className={`m-0 font-bold text-xs truncate ${s.is_featured ? 'text-[#f59e0b]' : 'text-white'}`}>
                            {s.is_featured ? '★ ' : ''}{s.title}
                          </p>
                          <p className="m-0 text-[10px] text-neutral-500 truncate mt-0.5">{s.speaker} • {s.category}</p>
                        </div>
                        <button onClick={() => handleDeleteSermon(s.id)} className="bg-red-500/10 border border-red-500/30 text-red-400 px-2.5 py-1 rounded-md text-[9px] font-bold uppercase hover:bg-red-500/20 transition cursor-pointer shrink-0">
                          Remove
                        </button>
                      </div>
                    ))
          ...
          {/* TAB 4: LIVE STREAM */}
          {activeTab === 'live' && (
            <div className="max-w-lg mx-auto bg-[#121212] border border-[#262626] p-6 rounded-xl shadow-xl">
              <div className="pb-3 mb-4 border-b border-[#262626]">
                <h2 className="text-[11px] font-bold text-white tracking-wide uppercase flex items-center space-x-1.5">
                  <span className="w-1.5 h-3.5 bg-[#f59e0b] rounded-full"></span>
              ...
              <form onSubmit={handleUpdateLive}>
                ...
                <label className="flex items-center space-x-2 text-neutral-300 text-[11px] mb-5 cursor-pointer">
                  <input type="checkbox" checked={isLiveActive} onChange={(e) => setIsLiveActive(e.target.checked)} className="w-3.5 h-3.5 accent-emerald-500 rounded cursor-pointer" />
                  <span>Service is Currently LIVE (Displays Live Badge on Website)</span>
                </label>

                <button type="submit" className="w-full py-2.5 bg-emerald-500 text-neutral-950 font-bold text-[11px] uppercase tracking-wider rounded-lg hover:bg-emerald-400 transition shadow-md shadow-emerald-500/10 cursor-pointer">
          ...
          {activeTab === 'audio' && (
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
              ...
              <div className="md:col-span-4 lg:col-span-4 bg-[#121212] border border-[#262626] p-4 rounded-xl shadow-xl flex flex-col">
                ...
                <div className="space-y-2.5 overflow-y-auto max-h-[420px] pr-1">
                  {audioList.length === 0 ? (
                    <p className="text-neutral-500 text-xs py-6 text-center">No audio tracks uploaded yet.</p>
                  ) : (
                    audioList.map((a) => (
                      <div key={a.id} className="bg-neutral-900/60 border border-neutral-800/80 p-3 rounded-lg flex justify-between items-center gap-2.5">
                        <div className="min-w-0 truncate">
                          <p className="m-0 font-bold text-xs text-white truncate">{a.title}</p>
                          <p className="m-0 text-[10px] text-neutral-500 truncate mt-0.5">{a.episode} • {a.duration}</p>
                        </div>
                        <button onClick={() => handleDeleteAudio(a.id)} className="bg-red-500/10 border border-red-500/30 text-red-400 px-2.5 py-1 rounded-md text-[9px] font-bold uppercase hover:bg-red-500/20 transition cursor-pointer shrink-0">
                          Remove
                        </button>
                      </div>
                    ))
